...
export async function handleChatCompletion(req: http.IncomingMessage, res: http.ServerResponse, bodyRaw: string) {
    ...
    try {
        ...
        if (proxyBody.tools && Array.isArray(proxyBody.tools)) {
            ...
            // Critical for Gemini 2.5 Fast (Error 400) and NomNom.
            // ALLOW Search for Gemini 3.0 (gemini, gemini-large) as requested.
            if ((actualModel.includes("gemini") && (actualModel.includes("fast") || actualModel.includes("legacy"))) || actualModel === "nomnom") {
